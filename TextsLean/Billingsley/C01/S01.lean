...
namespace Billingsley.C01.S01
/- 1.1.1 The unit interval -/

/- This text makes extensive use of the unit interval in the following guise -/
@[simp]
def UI := (Set.Ioc (0:ℝ) 1)

def badicInterval (b n i : ℕ) := Set.Ioc (i * (b:ℝ)⁻¹ ^ n) ((i + 1) * (b:ℝ)⁻¹ ^ n)

def badicIntervals (b n d : ℕ) := ⋃ k, Set.Ioc ((k * b + d) * (b:ℝ)⁻¹ ^ n) ((k * b + d + 1) * (b:ℝ)⁻¹ ^ n)
...
theorem eq_1_10 {b : ℕ} (hb : 1 < b) (u : ℕ → ℕ) (hu : Set.range u ⊆ Finset.range b) (n : ℕ) : (MeasureTheory.volume { x ∈ UI | ∀ i ∈ Finset.range n, facesLT b x i = u i }).toReal = (b:ℝ)⁻¹ ^ n := by
  rw [eq_1_9 hb u hu n, Real.volume_Ioc, add_sub_right_comm, sub_self, zero_add,
    ENNReal.toReal_ofReal (by positivity)
  ]

theorem eq_1_11 {b : ℕ} (hb : 1 < b) (n k : ℕ) (hk : k ≤ n) : (MeasureTheory.volume { x ∈ UI | ∑ i ∈ Finset.range n, facesLT b x i = k }).toReal = n.choose k * (b:ℝ)⁻¹ ^ n := by
  induction n
  · case zero =>
    interval_cases k
    simp only [UI, Finset.range_zero, Finset.sum_empty, and_true, Nat.choose_self,
      Nat.cast_one, pow_zero, mul_one, Set.setOf_mem_eq, Real.volume_Ioc, sub_zero]
    rw [ENNReal.toReal_ofReal (by norm_num)]
  · case succ n IH =>
    rcases k
    · simp only [Finset.sum_eq_zero_iff, Nat.choose_zero_right, Nat.cast_one, one_mul]
      exact eq_1_10 hb (fun _ ↦ 0) (by simp; positivity) (n + 1)
    case succ k =>
    by_cases hkn : k + 1 ≤ n
